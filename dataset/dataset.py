...
class MetaBric_Dataset(Dataset):
    def __init__(self, label_path, omics_paths, omics_types):
    ...
    def __getitem__(self, item):
        ERStatus = torch.Tensor(self.clinical_data['ERStatus'].values)
        HER2Status = torch.Tensor(self.clinical_data['HER2Status'].values)
        Pam50Subtype = torch.Tensor(self.clinical_data['Pam50Subtype'].values)
        Pam50SubtypeSingleLuminal = torch.Tensor(self.clinical_data['Pam50SubtypeSingleLuminal'].values)
        BasalNonBasal = torch.Tensor(self.clinical_data['BasalNonBasal'].values)
        Luminal = torch.Tensor(self.clinical_data['Luminal'].values)

        return self.omics, ERStatus, HER2Status, Pam50Subtype, Pam50SubtypeSingleLuminal, BasalNonBasal, Luminal
...
class Drug_response_Dataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        omics_data = {omics_type: torch.Tensor(df[df['ID'] == sample_id].values.tolist()[0][2:]) for omics_type, df in self.omics.items()}
